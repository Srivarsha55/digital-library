...
// Utility function to check if a book is a new arrival (updated in the last 30 days)
const isNewArrival = (updatedDate) => {
    const currentDate = new Date();
    const updated = new Date(updatedDate);
    const diffTime = Math.abs(currentDate - updated);
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    return diffDays <= 30; // Book is considered a new arrival if updated within the last 30 days
};

const NewArrivalsCarousel = ({ userMembership }) => {
    ...
    useEffect(() => {
        const fetchNewArrivals = async () => {
            try {
                ...
                const books = response.data.filter(book => isNewArrival(book.updated)); // Filter new arrivals based on update date
